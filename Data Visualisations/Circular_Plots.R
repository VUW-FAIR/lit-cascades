...
for(k in 11:length(cooc)){
    ...
    #prep clusters
    #i<-1
    clusters_df<-as.data.frame(c(1:length(clusters[,1])))
    ...
    cc<-as.data.frame(colframe$clusters)
    colnames(cc)[1]<-"clusters"
    colframe$colcode <- colcodes[cc$clusters]
